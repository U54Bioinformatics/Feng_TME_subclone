...
# run option
pyclone_sequenza_pre=true
pyclone_sequenza_run=true
...
min_alt_reads=5
min_vaf=0.05
min_callers=1 # 1 or 2 callers; run with 2 first, then use 1 or 2 to run pyclone: 1 without mincaller 2 with 
max_copy=2
cn=yes
germline=$prefix\_G
...
if [ $min_callers == 1 ]; then
    ...
    if [ $prefix == 'AB28' ] || [ $prefix == 'AB33' ]; then
