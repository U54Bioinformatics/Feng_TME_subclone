...
def read_mutation_effect_file(infile):
    data = defaultdict(lambda : int())
    file_df = pd.read_table(infile, header=0)
    for i in range(file_df.shape[0]):
        variant = '%s_%s' %(file_df['Chrom'][i], file_df['Pos'][i])
        # if Func.refGene is in ['ncRNA_exonic', 'splicing', 'ncRNA_splicing']
        var_effect = re.split(r';', str(file_df['Func.refGene'][i]))
        large_effect = 0
        for effect in var_effect:
            if file_df['Func.refGene'][i] in ['ncRNA_exonic', 'splicing', 'ncRNA_splicing']:
                large_effect = 1
        # if  ExonicFunc.refGene is in ['']
        if file_df['ExonicFunc.refGene'][i] in ['nonsynonymous SNV', 'stopgain', 'stoploss']:
            large_effect = 1
        if large_effect == 1:
            if file_df['ExonicFunc.refGene'][i] == '':
                data[variant] = '%s:NA' %(file_df['Func.refGene'][i])
            else:
                data[variant] = '%s:%s' %(file_df['Func.refGene'][i], file_df['ExonicFunc.refGene'][i])
    return data
...
def process_mutation_file(infile, sample, cluster, vaf, ccf, driver, effect, prefix):
    # ccf outfile
    ...
    print >> ofile_ccf, 'mutation\tcluster\tgene\teffect\tis.driver\t%s\t%s' %(sample_ccf, sample_vaf)  
    # vaf outfile
    ...
    print >> ofile_vaf, 'mutation\tcluster\tgene\teffect\tis.driver\t%s\t%s' %(sample_ccf, sample_vaf)   
    ...
    for i in range(file_df.shape[0]):
        ...
        if cluster.has_key(file_df['Cluster'][i]):
            ...
            isdriver = 'FALSE'
            # gene is a driver (cancer gene) and mutation has large effect
            if driver.has_key(gene) and effect.has_key(file_df['Mutation'][i]):
                isdriver = 'TRUE'
            if ccf.has_key(file_df['Mutation'][i]) and vaf.has_key(file_df['Mutation'][i]):
                ...
                newline = '%s\t%s\t%s\t%s\t%s\t%s\t%s' %(file_df['Mutation'][i], cluster_id, gene, effect[file_df['Mutation'][i]], isdriver, ccf_line, vaf_line)
...
def main():
    ...
    effect      = read_mutation_effect_file(effect_file)
    ...
    process_mutation_file(mutation_file, sample, cluster, vaf, ccf, driver, effect, args.output)
