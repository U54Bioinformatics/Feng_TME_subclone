...
def verify_mutation(mutation_list, infile):
    data = defaultdict(lambda : list())
    file_df = pd.read_table(infile, header=0)
    for i in range(file_df.shape[0]):
        #print file_df[0][i]
        mutation = '%s_%s' %(file_df['Chrom'][i], file_df['Pos'][i])
        if mutation_list.has_key(mutation):
            print '\t'.join(map(str, mutation_list[mutation]))
    return data


def read_mutation_list(infile):
    data = defaultdict(lambda : list())
    file_df = pd.read_table(infile, header=0)
    for i in range(file_df.shape[0]):
        #print file_df[0][i]
...
def main():
    ...
    verify_mutation(mutation_list, args.input)
