...
def parse_primer3(infile):
    ...
    s = re.compile(r'\=(.*)')
    ifile = open (infile, 'r')
    print "count\tlocus\tsize\tleft_tm\tright_tm\tprimerF_5'-3'\tprimerR_5'-3'"
    for line in ifile:
        ...
        if line.startswith('SEQUENCE_ID'):
            ...
            print '%s\t%s\t%s\t%s\t%s\t%s\t%s' % (count, locus, size, leftm, rightm, primerF, primerR)
            locus = ''
            size = ''
            leftm = ''
            rightm = ''
...
if __name__ == '__main__':
    ...
    for record in SeqIO.parse(args.input,"fasta"):
        ...
        if m:
            ...
            length2 = '140' if int(end) - int(start) < 200 else str(int(end) - int(start) + 1)
            length3 = 200
            if int(length) > 2000:
                length3 = int(length) + 200
...
PRIMER_THERMODYNAMIC_PARAMETERS_PATH=/home/jichen/Projects/Breast/scRNA/Feng/VariantCalls_primer_design/primer3-2.4.0/src/primer3_config
='''
        print >> ofile, cmd
    ofile.close()
   
    primer3 ='''
/home/jichen/software/BETSY/install/envs/Primer3/bin/primer3_core < primer3.infile > primer3.outfile
''' 
    os.system(primer3)    
