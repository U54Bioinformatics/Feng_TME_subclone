...
prefix=`cat Feng_patient.list | head -n $N | tail -n 1`
#prefix=FELINE
...
pyclone_FACETS_run=false

#filter variants SNP/INDEL
min_total_read=20
min_alt_reads=5
min_vaf=0.05
min_callers=2 # 1 or 2 callers
max_copy=4
cn=no
germline=$prefix\_G
pyclone_iterations=1000 # 1000 for testing run and 10000 for final run
...
if [ ! -e $prefix\_variant_calls_05_purity40_totalRD$min_total_read\_minread$min_alt_reads\_minVAF$min_vaf\_mincaller$min_callers\.txt ] ; then
...
#slice_svm.py --convert_to_position_file $prefix\_variant_calls_Germline_variant | grep -v "chrom" | awk '{print $1"\t"$2"\t"$2+1"\t"$3"\t"$4}' > $prefix\_variant_calls_Germline_variant.bed
    if [ $prefix == 'FEL044' ] || [ $prefix == 'FEL045' ] || [ $prefix == 'FEL046' ]; then
        python Prepare_somatic_variant_Germline_variants_FELINE.py --input $prefix\_variant_calls --germline_col 15
    else
        python Prepare_somatic_variant_Germline_variants_FELINE.py --input $prefix\_variant_calls --germline_col 17
    fi
...
# when zero will not use mincaller filter for pyclone
if [ $min_callers == 0 ]; then
    ...
    if [ $prefix == 'FEL045' ] || [ $prefix == 'FEL046' ]; then
...
# when 1 or 2 will use mincaller filter for pyclone
if [ $min_callers > 0 ]; then
